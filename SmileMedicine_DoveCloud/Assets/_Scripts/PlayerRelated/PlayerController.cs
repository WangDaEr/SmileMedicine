...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        if (inputLoackAcquired)
        {
            ...
            if (startSpecialMove && !startZMove)
            {
    ...
    public void StartSpecialMove(Vector3 des_pos, float switchSpeed, Vector3 des_scale)
    {
        ...
        this.switchSpeed = z_movementSpeed * Mathf.Sqrt((Mathf.Pow(des_pos.z - transform.position.z, 2.0F) + Mathf.Pow(des_pos.y - transform.position.y, 2.0F))) / Mathf.Abs(des_pos.z - transform.position.z);
        ...
        Debug.Log("Player start Special move))))))))))))))))))))))))))))))))) " + this.switchSpeed);
    }
    ...
    public void MovePlayerCharacter(Vector3 des)
    {
